module.exports = {
   ...
   */
  rules: {
    'vue/html-self-closing': [1, {              // 标签自闭合规则
        html: {
          void: 'never',                        // 空标签不自闭
          normal: 'always',                     // 自带标签如果没有内容自闭
          component: 'always',                  // 组件标签如果没有内容自闭
        }
    }],
    "vue/no-duplicate-attributes": 2,         // 不允许重复属性
    "vue/no-shared-component-data": 2,        // 限制data声明方式
    "vue/no-unused-components": 1,            // 声明组件必须使用
    "vue/no-use-v-if-with-v-for": 2,          // 禁止vif和for同时使用
    "vue/component-definition-name-casing": [1 , "kebab-case"], //组件命名方式，必须是 - 分开单词
    "vue/html-closing-bracket-newline": [1 , {
      "singleline": "never",                  // 单行属性标签最后的>号总是末尾
      "multiline": "always"                   // 多行属性标签最后的>号总是在属性下一行
    }],
    "vue/html-closing-bracket-spacing": [1, {
      "startTag": "never",                    // 标签开头不要空格
      "endTag": "never",                      // 非自闭标签末尾不要空格
      "selfClosingTag": "always"              // 自闭合标签末尾要空格
    }],
    "vue/html-indent": [1 ,"tab", {           // 属性有一个tab距离
      "attribute": 1,                         // 属性缩进倍数
      "baseIndent": 1,                        // 顶级标签缩进倍数
      "closeBracket": 0,                      // 结束标签不缩进
      "alignAttributesVertically": true,
      "ignores": []
    }],
    "vue/max-attributes-per-line": [1],       // 单行最多一个属性
    "vue/multiline-html-element-content-newline": [1, {
      "ignoreWhenEmpty": true,
      "allowEmptyLines": false,               // 尾标签必须于内容换行
    ...
    "no-return-await": 1,                    // async函数中禁止return await
